...
async function getRefreshToken(token) {
    ...
    if (!refreshToken || !refreshToken.isActive)
        throw 'Invalid token'; // ✅ fixed inverted logic (was refreshToken.isActive)
    return refreshToken;
}
...
function generateJwtToken(account) {
    return jsonwebtoken_1.default.sign({ sub: account.id, role: account.role }, process.env.JWT_SECRET, { expiresIn: '15m' });
}
