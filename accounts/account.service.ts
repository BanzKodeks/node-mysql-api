...
async function verifyEmail({ token }: any) {
    const account = await db.Account.findOne({ where: { verificationToken: token } }); // ✅ fixed typo (was verficationToken)
...
async function forgotPassword({ email }: any, origin: string) {
    ...
    await account.save(); // ✅ fixed: was missing before
...
async function getRefreshToken(token: any) {
    ...
    if (!refreshToken || !refreshToken.isActive) throw 'Invalid token'; // ✅ fixed inverted logic (was refreshToken.isActive)
    return refreshToken;
}
...
function generateJwtToken(account: any) {
    return jwt.sign( { sub: account.id, role: account.role }, process.env.JWT_SECRET as string, { expiresIn: '15m' });
}
